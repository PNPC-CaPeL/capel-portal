...
export default defineNuxtPlugin(async () => {
  ...
  // Side client we need to use public
  // const client = new lckClientServer(
  //   config.public.LCK_BASE_PATH,
  //   config.public.LCK_DB_UUID,
  //   config.public.LCK_USERNAME,
  //   config.public.LCK_PASSWORD,
  // )

  const fetchConnection =
    typeof window !== 'undefined' ? window.fetch.bind(window) : fetch

  const lckRestClient = restClient(config.LCK_BASE_PATH)

  const client = createClient(lckRestClient.fetch(fetchConnection))

  console.log('res', config.LCK_USERNAME, config.LCK_PASSWORD)

  const user = await client.authenticate({
    strategy: 'local',
    email: config.LCK_USERNAME,
    password: config.LCK_PASSWORD,
  })
